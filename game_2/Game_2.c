/**
 * @file    Game_2.c
 * @brief   Ultimate Flight - integrated into the group menu framework
 *
 * Refactor notes (based on the original Stage 1 main.c):
 *   1. Entry point changed from int main(void) to MenuState Game2_Run(void)
 *   2. Removed all HAL_Init / SystemClock_Config / MX_*_Init / Error_Handler / _write
 *      (these are now performed in Core/main.c)
 *   3. cfg0 / buzzer_cfg are declared as extern (shared resources, already
 *      initialised in main.c)
 *   4. joystick_cfg / joystick_data / pwm_cfg are declared static (private
 *      peripherals owned by this game only)
 *   5. Direct GPIO read on PC2 replaced with current_input.btn2_pressed
 *      (uniform input handling provided by the framework)
 *   6. Added the framework-defined "BT3 returns to main menu" exit logic
 *   7. All internal functions and globals are static, to avoid linker
 *      conflicts with Game_1 / Game_3
 *   8. GameState_t renamed to Game2State_t to avoid type-name conflicts
 */
 ...
 *  External resources
 *  ------------------------------------------------------------
 *  HAL handles generated by CubeMX in Core/main.c, plus the LCD
 *  and Buzzer configurations shared by the whole project, are
 *  referenced via extern. They are NEVER re-initialised here.
 * ============================================================ */
...
/* ============================================================
 *  Peripherals private to this game (not shared, hence static)
 *  ------------------------------------------------------------
 *  Joystick_Init / PWM_Init use an internal setup_done flag,
 *  so calling them again is idempotent and safe at every
 *  Game2_Run() entry.
 * ============================================================ */
