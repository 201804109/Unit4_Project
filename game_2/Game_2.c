...
extern ST7789V2_cfg_t  cfg0;          /* shared LCD     */
extern Buzzer_cfg_t    buzzer_cfg;    /* shared buzzer  */
...
/* Colour palette indices: 0=black 1=white 2=red 3=green 7=grey 8=purple 10=gold */
...
{
    /* 1. One-shot init for game-private peripherals (idempotent) */
    Joystick_Init(&joystick_cfg);
    Joystick_Calibrate(&joystick_cfg);
    PWM_Init(&pwm_cfg);

    /* 2. Reset the state machine: every entry starts at the title screen */
    game_state = G2_TITLE;
    LCD_Fill_Buffer(0);
    render_title();
    play_startup_sound();
    ...
    MenuState exit_state      = MENU_STATE_HOME;   /* default: back to menu */

    /* 3. Main game loop */
    while (1)
    {
        /* Non-blocking auto-off for buzzer / LED */
        ...
        /* Read inputs: framework buttons + joystick */
        Input_Read();
        ...
        /* Universal exit: BT3 returns to main menu in any state
        ...
        switch (game_state)
        {
            ...
            case G2_PLAYING:
                /* BT2 toggles pause (replaces the original direct PC2 read) */
            ...
            case G2_PAUSED:
                ...
                if (current_input.btn2_pressed && joystick_released) {
                    ...
                    /* Time compensation: add the pause duration back into
                     * every time-based counter so that gameplay timing
    ...
    /* 4. Cleanup before returning so we leave no buzzer/LED hanging on */
    buzzer_off(&buzzer_cfg);
    PWM_Off(&pwm_cfg);
    return exit_state;
}
...
static void update_game(void)
{
    /* 1. Move the player horizontally with the joystick */
    ...
    /* 2. Every 10 seconds: bump the difficulty and trigger
     *    the rising "SPEED UP!" text animation */
    ...
    /* 3. Spawn an obstacle on a fixed frame interval */
...
/* ============================================================
 *  Spawn a new obstacle (uses hardware RNG for X position)
 * ============================================================ */
static void spawn_obstacle(void)
{
    for (int i = 0; i < OBS_MAX_COUNT; i++) {
    ...
    for (int i = 0; i < OBS_MAX_COUNT; i++) {
        ...
        if (overlap_x && overlap_y) {
            ...
            if (lives <= 0) {
                ...
                /* LED solid for 1 s indicates game over */
                PWM_Set(&pwm_cfg, 1000, 100);
                led_off_time = HAL_GetTick() + 1000;

                joystick_released = 0;     /* force the player to release the stick */
                ...
                /* LED short flash 300 ms indicates losing one life */
                PWM_Set(&pwm_cfg, 1000, 100);
...
static void render_game(void)
{
    ...
    /* 5. Small hint at top-right: "BT3:Menu" */
...
static void render_title(void)
{
    LCD_Draw_Rect(0,   0, 240, 120, 8, 1);  /* upper half: purple */
    LCD_Draw_Rect(0, 120, 240, 120, 3, 1);  /* lower half: green  */
    ...
    /* Drawn twice with a 1-pixel offset to fake bold text */
    ...
    /* Bomb decoration: grey body, matches the in-game bombs */
...
/* ============================================================
 *  Sound effects
 *  (non-blocking; the buzzer is auto-disabled in the main loop
 *   when HAL_GetTick() reaches buzzer_off_time)
 * ============================================================ */
static void play_startup_sound(void)
{
